...
function EntityController($scope, $http, $rootScope, $location, $window, $timeout, $stateParams, SearchService, $filter, anchorSmoothScroll, Lightbox, EntityService, $state) {
    ...
    $scope.addFirstTime = function (chkName,index) {
        console.log($('#' + chkName).is(':checked'));
        if ($('#' + chkName).is(':checked')) {
            $scope.hours[$scope.weekDays[index]].push({open_time: "10:00",openampm:'AM', close_time: "22:00",closeampm:'PM'});
        }
        else {
            $scope.hours[$scope.weekDays[index]] = [];
        }
    }
    //
    $scope.addNewTime = function (index) {
        console.log("add new time " + index)
        $scope.hours[$scope.weekDays[index]].push({open_time: "10:00",openampm:'AM', close_time: "22:00",closeampm:'PM'});
    }
    //
    $scope.removeSelectedTime = function (index, pindex,chkname) {
        console.log("add new time " + index)
        $scope.hours[$scope.weekDays[pindex]].splice(index, 1);
        if ($scope.hours[$scope.weekDays[pindex]].length === 0) {
            $('#' + chkname).attr('checked', false);
        }
    }
//
    $scope.clearHours = function (index) {
        $scope.hours = {Monday: [], Tuesday: [], Wednesday: [], Thursday: [], Friday: [], Saturday: [], Sunday: []};
        for (var i = 0; i < 7; i++) {
            $('#checkbox' + $scope.weekDays[i]).attr('checked', false);
        }
    }
//
    $scope.saveHours = function (entityType, actionType) {
        var tmpObj = angular.copy($scope.hours);
        angular.forEach(tmpObj, function (item, index) {
            console.log(item.length + " = " + index)
            if (item.length === 0) {
                delete $scope.hours[index];
            }
            angular.forEach(item, function (subitem) {
                delete subitem["$$hashKey"]
            });
            angular.forEach(item, function (subitem) {
                subitem["open_time"] = subitem["open_time"] + " " + subitem["openampm"];
                subitem["close_time"] = subitem["close_time"] + " " + subitem["closeampm"];
                delete subitem["openampm"];
                delete subitem["closeampm"];
            });
        });
        $scope.savePlaceHours(entityType, actionType, JSON.stringify(tmpObj));
        console.log($scope.hours);
    }
    //
    $scope.resetHours = function()
    {
        angular.forEach($scope.itemDetails.hours,function(item,indx){
            $scope.hours[item.day_of_week.toLowerCase()] = [];
            angular.forEach(item.hours,function(hourItem,ind){
                var openTime = hourItem.open_time;
                var closeTime = hourItem.close_time ;
                $scope.hours[item.day_of_week.toLowerCase()].push({open_time:openTime.substr(0,hourItem.open_time.length-2),openampm:openTime.substr(hourItem.open_time.length-2).trim(),close_time:closeTime .substr(0,hourItem.close_time.length-2),closeampm:closeTime.substr(hourItem.close_time.length-2).trim()})
            })
        })
    }
    //
    $scope.weekDays = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
    $scope.weekDaysInSort = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    $scope.showHideEdit = function () {
        ...
        //angular.forEach($scope.itemDetails.hours,function(item,indx){
        //    //console.log(item.day_of_week.toLowerCase());
        //    //console.log(item.hours.length);
        //    $scope.hours[item.day_of_week.toLowerCase()] = item.hours
        //
        //})
        ...
        $scope.placeAddress = {
            streatadd:$scope.itemDetails.mapinfo.address.street_address,
            city:$scope.itemDetails.mapinfo.address.city,
            state:$scope.itemDetails.mapinfo.address.state,
            country:$scope.itemDetails.mapinfo.address.country,
            postalCode:$scope.itemDetails.mapinfo.address.postal_code
            //pobox:itemDetails.mapinfo.address.street_address
        }
        $scope.newData = {
            "newValue": "",
...
//
    $scope.savePlaceHours = function (entityType, actionType, data) {
    ...
    $scope.savePlaceAddress = function (placeAddress, entityType, actionType) {
        ...
        newValue.newValue = JSON.stringify(tmpObj);
    ...
    $scope.entityEditor = function (entityType, actionType,newVal) {
        $scope.newData['newValue'] = newVal;
    ...
    $scope.editAction = function (actiomItem, opt) {
        ...
        if (opt === 'DELETE') {
            delete newValue.newValue;
            console.log('new ', JSON.stringify(newValue));
            $http.defaults.headers.common = "{'Content-Type':'application/json'}";
            var url = "http://nedit-w.relcy.com/" + entityType + "/" + $scope.itemDetails.title + "/" + actiomItem.type
